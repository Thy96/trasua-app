...
function ProductDetail({ showPopUp, setShowPopUp, showProductDetail, callbackOrderSuccess }) {
    const [productDetail, setProductDetail] = useState(getProductListToLocalStore())
    const [selectedPrice, setSelectedPrice] = useState(null);
    const [selectedTopping, setSelectedTopping] = useState([])
    const [countQty, setCountQty] = useState(1);

    const [productSucess, setProductSucess] = useState([])
    ...
    const setLocalStoreProductDetail = (product, secSize) => {
    ...
    useEffect(() => {
        getProductListToLocalStore()
        setProductSucess(JSON.parse(localStorage.getItem('list_products')))
    }, [productDetail])

    console.log("product detail ", productSucess);
    callbackOrderSuccess(productSucess)
        ...
        <Popup open={showPopUp} closeOnDocumentClick onClose={closePopup}>
            <div className={cx('modal')}>
                ...
                <div className={cx('product-flex')}>
                    <figure><img src={showProductDetail.url} alt={showProductDetail.name} /></figure>
