...
function ProductCart({ orderListCart }) {
    ...
    const [infoCustomer, setInfoCustomer] = useState(() => {
        const newCustomer = JSON.parse(localStorage.getItem("List_Info_Customer"));
        return newCustomer ?? [];
    });

    const [productCart, setProductCart] = useState(() => {
        const newCustomer = JSON.parse(localStorage.getItem("list_products"));
        return newCustomer ?? [];
    })

    useEffect(() => {
        setProductCart(orderListCart)
        handleData(productCart)
    }, [productCart, orderListCart])


    console.log("product Cart ", orderListCart);
    console.log("Local product Cart ", productCart);
    ...
    function handleData(productCart) {
        ...
        const priceItem = tempArr.map((arr) => arr.qty * arr.price + arr.totalTopping)
    ...
    const handleDeletedItem = (index) => {
        // json_product.filter(item => item_id.id !== index);
        console.log(index);
    }
                                    ...
                                    return (
                                        <dl key={item.id} className={cx("content-flex")}>
                                            ...
                                            <dd className={cx("content-flex-note")}>
                                                ...
                                                Topping: {item.selectedTopping.map(size => (
                                                    <p>- {size.name}</p>
                                                ))}
                                            </dd>
